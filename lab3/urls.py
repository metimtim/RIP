...
urlpatterns = [
    ...
    path(r'parking/<int:id_parking>/', views.GetParking.as_view(), name='get-parking-by-id'),
    # получить конкретную заявку (GET),
    path(r'parking/<int:id_parking>/', views.GetParking.as_view(), name='put-parking-by-id'),
    # изменить конкретную заявку (PUT),

    path(r'form-parking/<int:id_parking>/', views.FormParking.as_view(), name='form-parking-by-id'),
    # формирование заявки (PUT)
    path(r'moderate-parking/<int:id_parking>/', views.ModerateParking.as_view(), name='moderate-parking-by-id'),
    # завершить/отклонить модератором (PUT)
    path(r'delete-parking/<int:id_parking>/', views.ModerateParking.as_view(), name='delete-parking-by-id'),
    # удалить заявку (DELETE)

    # m-m
    path(r'delete-from-parking/<int:id_parking>/ship/<int:id_ship>/', views.EditShipParking.as_view(),
         name='delete-from-parking-by-id'),
    # удалить из заявки (DELETE)
    path(r'add-captain/<int:id_parking>/ship/<int:id_ship>/', views.EditShipParking.as_view(),
         name='add-captain-request-by-id'),
